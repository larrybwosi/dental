...
pub async fn start_spoke_client(app_handle: AppHandle, pairing_code: String, manual_addr: Option<String>) {
            ...
            if let ServiceEvent::ServiceResolved(info) = event {
                ...
                if let Some(addr) = addr {
                    if let Ok(mut current_hub) = hub_addr_clone.lock() {
                        if let Some(hub_url) = current_hub.as_ref() {
                             // Already have a hub
                             debug!("Hub already discovered: {}", hub_url);
                        } else {
                            let hub_url = format!("{}:{}", addr, info.get_port());
                            *current_hub = Some(hub_url.clone());
    ...
    tokio::spawn(async move {
        loop {
            let (addr, token) = {
                let addr_res = hub_addr_ws.lock();
                let token_res = pairing_token_ws.lock();
                match (addr_res, token_res) {
